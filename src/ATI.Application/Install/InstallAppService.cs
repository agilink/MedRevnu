...
using ATI.Migrations.Seed.Host;
...
namespace ATI.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : ATIAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<ATIDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{ATIConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{ATIConsts.ConnectionStringName}", constring);
        }
