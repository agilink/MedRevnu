...
var chat = {
    ...
    getFormattedMessages: function (messages) {
        $.each(messages, function (index, message) {
            ...
            if (message.message.startsWith('[image]')) {
                ...
                var imageUrl = abp.appPath + 'Core/Chat/GetImage?id=' + message.id + '&contentType=' + image.contentType;
                ...
                var fileUrl = abp.appPath + 'Core/Chat/GetFile?id=' + message.id + '&contentType=' + file.contentType;
                var uploadedFileMsg =
                    '<a href="' +
                    fileUrl +
    ...
    user: {
        ...
        setFriendOnlineStatus: function (userId, tenantId, isOnline) {
            ...
            var statusClass = 'symbol-badge bg-' + (isOnline ? 'success' : 'secondary') + ' start-100 top-100 border-4 h-15px w-15px ms-n2 mt-n2';
...
(function ($) {
    $(function () {
        ...
        var url = abp.appPath + 'Core/Chat/UploadFile';
        ...
        dropZone.on("success", function (file, response) {
            ...
            $(imageId).prop('disabled', !$.support.fileInput)
                .parent()
        ...
        dropZone.on("success", function (file, response) {
            ...
            $(fileId).prop('disabled', !$.support.fileInput)
                .parent()
                .addClass($.support.fileInput ? undefined : 'disabled');
        });
