...
using ATI.Authentication.TwoFactor.Google;
...
using ATI.Authorization.Impersonation;
...
using ATI.Web.Models.Account;
using ATI.Security;
using ATI.Security.Recaptcha;
using ATI.Sessions;
using ATI.Url;
using ATI.Web.Authentication.External;
using ATI.Web.Security.Recaptcha;
using ATI.Web.Session;
using ATI.Web.Views.Shared.Components.TenantChange;
...
using ATI.Authentication.PasswordlessLogin;
using ATI.Authorization.PasswordlessLogin;

namespace ATI.Web.Controllers
{
    public class AccountController : ATIControllerBase
    {
            ...
            string successMessage = "", string ss = "")
        {
            ...
            return View(
                new LoginFormViewModel
                {
                    ...
                    IsPasswordlessLoginEnabled = IsPasswordlessLoginEnabled(),
            ...
            string returnUrlHash = "", string ss = "")
        {
            ...
            if (loginResult.User.ShouldChangePasswordOnNextLogin)
            {
                ...
                var expireDate =
                    Uri.EscapeDataString(Clock.Now.AddHours(1).ToString(ATIConsts.DateTimeOffsetFormat));
        ...
        public ActionResult PasswordlessLogin()
        {
            ...
            {
                Providers = providers,
                IsEmailPasswordlessLoginEnabled = IsEmailPasswordlessLoginEnabled(),
                IsSmsPasswordlessLoginEnabled = IsSmsPasswordlessLoginEnabled(),
                SelectedProvider = GetDefaultSelectedProvider()
            });
        }
        ...
        {
            if (model.SelectedProviderValue.IsNullOrEmpty() || !IsPasswordlessLoginEnabled())
            {
                return RedirectToAction("Login");
            }
        ...
        private string GetDefaultSelectedProvider()
        {
            bool isEmailEnabled = IsEmailPasswordlessLoginEnabled();
            bool isSmsEnabled = IsSmsPasswordlessLoginEnabled();
        ...
        {
            if (IsEmailPasswordlessLoginEnabled() || IsSmsPasswordlessLoginEnabled())
            {
                return true;
            }
        ...
        private bool IsEmailPasswordlessLoginEnabled()
        {
            return SettingManager.GetSettingValue<bool>(AppSettings.UserManagement.PasswordlessLogin
                .IsEmailPasswordlessLoginEnabled);
        }

        private bool IsSmsPasswordlessLoginEnabled()
        {
            return SettingManager.GetSettingValue<bool>(AppSettings.UserManagement.PasswordlessLogin
        ...
        {
            return RedirectToAction("Index", "Home", new {area = "Core"});
        }

        public string GetAppHomeUrl()
        {
            return Url.Action("Index", "Home", new {area = "Core"});
        }
